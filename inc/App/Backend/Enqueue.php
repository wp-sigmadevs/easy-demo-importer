	...
	private function localizeData() {
		return [
			...
			'data'   => [
				...
				'snapshotDetails'            => esc_html__( 'Saves a full backup — content, media files, and settings — before importing. One click restores this exact state from the result screen or the restore-point banner. Rolling back also removes anything created after the import. Storage: a duplicate copy of every table this plugin can touch is written alongside your live ones, so your database roughly doubles in size until the restore point is discarded. Media files are recorded rather than copied, so they add no extra disk use.', 'easy-demo-importer' ),
