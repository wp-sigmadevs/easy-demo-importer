...
const FEATURES = [
	{
		...
		label: 'Chunked streaming XML import — no timeouts',
	},
...
const WelcomeStep = () => {
	...
	useEffect(() => {
		const el = document.getElementById('edi-wizard-next-slot');
		if (el) {
			setFooter(el);
		}
	}, []);
		...
		<>
			<div style={{ maxWidth: 540 }}>
				...
				<p style={{ color: '#595959', marginBottom: 28, fontSize: 15 }}>
					Import a complete demo in a few guided steps. The process
					takes roughly <strong>2–5 minutes</strong> depending on
					server speed and demo size.
				</p>
				...
				<Space direction="vertical" size={12} style={{ width: '100%' }}>
					{FEATURES.map((f, i) => (
						...
						>
							<span style={{ color: '#6366f1', fontSize: 16 }}>
								{f.icon}
							</span>
