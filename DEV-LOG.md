...
**Known follow-ups:** ~~a permanently-failing download leaves one `.part`/`.chunk` in the demo upload dir with no sweeper~~ — resolved: `ManualImport::cleanupStale()` now also sweeps `imported-demo-data.zip.<hash>.{part,chunk}` past the TTL. It owns this because it is the only cron that loads unconditionally (`App\Manual` is ungated in `Classes.php`; `App\Ajax\Backend` is `onRequest => import`, so `DownloadFiles` cannot host a cron). Still open: same-URL-file-replaced-mid-download resumes from a stale offset (needs `If-Range`/ETag); `appendChunk`'s `@since 2.1.0` assumes the next release version.
